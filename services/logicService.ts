...
const LIVE_VACCINES = ['MMR', 'Varicella', 'Measles', 'Mumps', 'Rubella', 'FluMist', 'Zoster', 'Rotavirus']; 
...
export const evaluateSeries = (
  ...
  if (nextRule.maxAgeMonths && currentAgeMonths > nextRule.maxAgeMonths) {
    ...
    if (count > 0) {
         // Special Safety Logic for Rotavirus: 
         // If a child ages out of the series window, it is CONTRAINDICATED to continue due to intussusception risk,
         // whereas for other vaccines it is simply COMPLETE (aged out).
         const status = seriesName === 'Rotavirus' ? VaxStatus.CONTRAINDICATED : VaxStatus.COMPLETE;
