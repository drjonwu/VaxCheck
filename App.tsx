...
const App: React.FC = () => {
  ...
  const loadPatients = useAppStore(state => state.loadPatients); // ACTION
  const isLoadingPatients = useAppStore(state => state.isLoadingPatients); // STATE
  ...
  // Ref to track the currently mounted patient ID to prevent race conditions during async streaming
  const activePatientIdRef = useRef(currentPatient.profile.id);
  ...
  // EFFECT: Reset local UI state when patient changes to prevent stale data
  useEffect(() => {
    // Only reset if ID actually changed
    if (activePatientIdRef.current !== currentPatient.profile.id) {
        activePatientIdRef.current = currentPatient.profile.id;
        setExplanation({});
        setGenerating(null);
        setExpandedRow(null);
    }
  }, [currentPatient.profile.id]);

  // EFFECT: Run Analysis when Patient or Rules Change
  useEffect(() => {
    let mounted = true;
    if (currentPatient.profile.id === 'loading') return; // Skip analysis for dummy patient
  ...
  const handleExplain = useCallback(async (seriesStatus: VaccineSeriesStatus) => {
    // Capture the ID of the patient we are requesting an explanation for
    const requestingPatientId = currentPatient.profile.id;
    ...
    await streamClinicalExplanation(
      ...
      seriesForecast,
      (text) => {
        // CRITICAL: Guard against race condition.
        // If the user has switched patients since the request started, ignore the incoming chunks.
        if (activePatientIdRef.current !== requestingPatientId) {
            return;
        }
    ...
    // Only clear loading state if we are still on the same patient
    if (activePatientIdRef.current === requestingPatientId) {
        setGenerating(null);
    }
  ...
  if (isLoadingPatients && patients.length === 0) {
      return (
          <div className="flex h-screen w-full items-center justify-center bg-slate-50 flex-col gap-4">
              ...
              <div className="text-slate-500 font-medium animate-pulse">Initializing VaxCheck Clinical Engine...</div>
          </div>
      );
  }
    ...
    <div className="flex h-screen bg-slate-100 font-sans text-slate-900 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-100 h-full relative">
        ...
        <div className="flex-1 overflow-hidden relative flex flex-col">
            ...
            {currentView === 'patient' && (
                <>
                    <PatientHeader patient={currentPatient.profile} gapAnalysis={filteredGapAnalysis} />
                        ...
                        <div className="bg-white p-4 rounded-sm border border-slate-300 shadow-sm">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {/* Legend - Updated with full-size icons */}
                                    <div className="hidden xl:flex flex-wrap gap-4 bg-slate-50 px-3 py-1.5 rounded border border-slate-100 ml-2">
                                        <span className="flex items-center gap-1.5 text-xs text-slate-600 font-medium">
                                          <div className="w-5 h-5 rounded-full bg-emerald-600 border border-white flex items-center justify-center text-white">
                                              <Check size={10} strokeWidth={3}/>
                                          </div>
                                          Given
                                        </span>
                                        <span className="flex items-center gap-1.5 text-xs text-slate-600 font-medium">
                                          <div className="w-5 h-5 rounded-full bg-amber-600 border border-white flex items-center justify-center text-white">
                                              <AlertTriangle size={10} strokeWidth={3}/>
                                          </div>
                                          Due
                                        </span>
                                        <span className="flex items-center gap-1.5 text-xs text-slate-600 font-medium">
                                          <div className="w-5 h-5 rounded-full bg-red-600 border border-white flex items-center justify-center text-white">
                                              <AlertCircle size={10} strokeWidth={3}/>
                                          </div>
                                          Overdue
                                        </span>
                                        <span className="flex items-center gap-1.5 text-xs text-slate-600 font-medium">
                                          <div className="w-5 h-5 rounded-full bg-white border border-slate-300 border-dashed flex items-center justify-center text-[9px] font-bold text-slate-400">
                                              #
                                          </div>
                                          Plan
                                        </span>
                            ...
                            <div className={`bg-slate-50 rounded border border-slate-200 overflow-hidden relative ${filteredGapAnalysis.length > 0 ? '' : 'min-h-[300px]'}`}>
                ...
                <div className="p-6 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div className="mt-1 text-slate-900">Gemini 3 Flash (Privacy Mode)</div>
                        </div>
                    </div>
                </div>
            </div>
